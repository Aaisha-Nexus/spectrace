...
AgentEventCallback = Callable[[AgentNode, str, TrajectoryEvent | None], None]
...
def _notify_event(
    ...
    """Publish a concise execution update without affecting headless behavior."""

    state.current_node = node
    if callback is not None:
        callback(node, status, event)
...
def _append_event(
    ...
    *,
    ...
    _notify_event(
        ...
        state,
        node,
        "PAUSED" if node == AgentNode.AWAIT_HUMAN_REVIEW else "COMPLETED",
        event,
    )
...
def _run_until_human_review_impl(
    state: AdvancedRunState,
    ledger: LedgerStore,
    ...
    *,
    ...
    _notify_event(event_callback, state, AgentNode.LOAD_SCOPE_ANCHOR, "RUNNING")
    ...
    _append_event(state, ledger, AgentNode.LOAD_SCOPE_ANCHOR, tool="build_scope_anchor", input_ids=(state.project_id,), input_value=state.project_pack_path, summary=f"Loaded {len(anchor.items)} evidence items.", started=started, event_callback=event_callback)

    _notify_event(event_callback, state, AgentNode.RETRIEVE_EVIDENCE, "RUNNING")
    started = time.perf_counter()
    ...
    _append_event(state, ledger, AgentNode.RETRIEVE_EVIDENCE, tool="retrieve_evidence", input_ids=(state.request.request_id, *evidence_ids), input_value={"message": state.request.message, "cutoff": state.request.evidence_available_through}, summary=f"Retrieved {len(evidence_ids)} cutoff-safe evidence items.", started=started, event_callback=event_callback)

    _notify_event(event_callback, state, AgentNode.ASSESS_SUFFICIENCY, "RUNNING")
    started = time.perf_counter()
    sufficiency = assess_sufficiency(state.request, retrieval, anchor)
    state.sufficiency = sufficiency
    _append_event(state, ledger, AgentNode.ASSESS_SUFFICIENCY, tool="assess_sufficiency", input_ids=(state.request.request_id,), input_value=sufficiency.model_dump(mode="json"), summary="Classification sufficiency assessed.", started=started, event_callback=event_callback)

    _notify_event(event_callback, state, AgentNode.CHECK_CONTRADICTIONS, "RUNNING")
    started = time.perf_counter()
    conflicts = find_effective_conflicts(state.request, retrieval, anchor)
    state.conflicts = conflicts
    _append_event(state, ledger, AgentNode.CHECK_CONTRADICTIONS, tool="find_effective_conflicts", input_ids=(state.request.request_id, *conflicts.conflicting_evidence_ids), input_value=conflicts.model_dump(mode="json"), summary="Current effective conflicts and boundaries checked.", started=started, event_callback=event_callback)

    _notify_event(event_callback, state, AgentNode.CLASSIFY_REQUEST, "RUNNING")
    started = time.perf_counter()
    ...
    _notify_event(event_callback, state, AgentNode.CALCULATE_CUMULATIVE_DRIFT, "RUNNING")
    started = time.perf_counter()
    ...
    state.drift = drift
    _append_event(state, ledger, AgentNode.CALCULATE_CUMULATIVE_DRIFT, tool="calculate_cumulative_drift", input_ids=(state.request.request_id, *drift.related_decision_ids), input_value=[record["entry_hash"] for record in records], summary=f"Drift severity: {drift.severity.value}.", started=started, event_callback=event_callback)

    _notify_event(event_callback, state, AgentNode.VERIFY_ASSESSMENT, "RUNNING")
    started = time.perf_counter()
    eligible_requests = (
    ...
    _notify_event(event_callback, state, AgentNode.PREPARE_RECOMMENDATION, "RUNNING")
    started = time.perf_counter()
    action = (
        HumanAction.DEFER
    ...
    _append_event(state, ledger, AgentNode.PREPARE_RECOMMENDATION, tool="prepare_human_recommendation", input_ids=(state.request.request_id,), input_value=recommendation.model_dump(mode="json"), summary=f"Prepared {action.value} recommendation.", started=started, event_callback=event_callback)

    state.pause_snapshot_hash = ledger.snapshot(state.project_id).snapshot_hash
    state.status = AgentStatus.AWAITING_HUMAN_REVIEW
    _notify_event(event_callback, state, AgentNode.AWAIT_HUMAN_REVIEW, "RUNNING")
    started = time.perf_counter()
    _append_event(state, ledger, AgentNode.AWAIT_HUMAN_REVIEW, tool=None, input_ids=(state.request.request_id, assessment_id), input_value=state.pause_snapshot_hash, summary="Paused before consequential ledger mutation.", started=started, human_state=state.status.value, event_callback=event_callback)
    return state
...
def run_until_human_review(
    ...
    *,
    ...
    """Run to the human gate and optionally publish truthful node transitions."""

    try:
        return _run_until_human_review_impl(
            state,
            ledger,
            client,
            limits=limits,
        ...
        if state.current_node is not None:
            _notify_event(event_callback, state, state.current_node, "FAILED")
...
def resume_after_human_review(
    ...
    *,
    ...
    try:
        _notify_event(event_callback, state, AgentNode.APPLY_HUMAN_DECISION, "RUNNING")
        started = time.perf_counter()
        ...
        _append_event(state, ledger, AgentNode.APPLY_HUMAN_DECISION, tool="apply_human_review", input_ids=(review.review_id, review.request_id), input_value=review.model_dump(mode="json"), summary="Applied explicit human review transaction.", started=started, human_state=review.action.value, event_callback=event_callback)

        _notify_event(event_callback, state, AgentNode.BUILD_CHANGE_IMPACT_PACKAGE, "RUNNING")
        started = time.perf_counter()
        ...
        _notify_event(event_callback, state, AgentNode.COMPLETE, "RUNNING")
        state.status = AgentStatus.COMPLETE
        started = time.perf_counter()
        _append_event(state, ledger, AgentNode.COMPLETE, tool=None, input_ids=(state.request.request_id,), input_value=ledger.snapshot(state.project_id).snapshot_hash, summary="Advanced run completed after human review.", started=started, human_state=state.status.value, event_callback=event_callback)
        return state
    except Exception:
        if state.current_node is not None:
            _notify_event(event_callback, state, state.current_node, "FAILED")
